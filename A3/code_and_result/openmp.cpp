...
int main(int argc, char *argv[]){
    ...
    int num_bodies = argv[2] ? atoi(argv[2]):200;
...
#pragma omp parallel for private(bodyme) private(tempF) 
        for (i = 0; i < num_bodies; i++) {
            bodyme.x = bodylist[i].x;
            bodyme.y = bodylist[i].y;
            bodyme.mass = bodylist[i].mass;
            Force F;
            // calculate_force_routines
...
#pragma omp parallel for 
    ...
    printf("OpenMP: Threads: %d; Number of bodies: %d; Runtime: %f\n",num_threads, num_bodies, Time);
    // printf("a: %d; b: %d;\n",a, b);
}
...
Force cal_force_two(Body b1, Body b2) {
    ...
    resultF.x = (6.674*b1.mass*b2.mass/r_sq)*(b2.x-b1.x)/sqrt(r_sq);
    resultF.y = (6.674*b1.mass*b2.mass/r_sq)*(b2.y-b1.y)/sqrt(r_sq);
    return resultF;
}
